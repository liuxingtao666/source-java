...
//Step1.3:将提示框的className属性设置为""，表示以普通样式显示即可。
...
function validateUserName(txtObj,divID){
	...
	//如果验证通过
	if(!isRight){		
		//将提示信息框的样式更换为在CSS中专门定义的验证失败的样式名称。
...
function validateDatas(){
	//验证姓名文本框。
	//1、先获得姓名文本框
	//2、然后调用validateUserName方法，将姓名文本框对象和它对应的提示信息框id作为参数传递到方法中。
	//3、遇到一个问题，我们怎么知道validateUserName方法验证通过了呢？只能通过方法的返回值。所以这里需要在每个验证方法最后，增加一个返回值return isRight。来返回正则表达式验证的结果。
	var txtObj = document.getElementById("userName");
	var rName = validateUserName(txtObj, "userName_msg");
	//注意，这句和onclick中的return，联合使用，可以在验证不通过时取消提交。保持提示信息在页面。两者缺一不可。
	return rName;
}